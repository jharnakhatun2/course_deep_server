...
router.get("/user/:email", async (req, res) => {
  try {
    ...
    const bookings = await bookingsDatabase.find({ userEmail: email }).toArray();
    res.json(bookings);
  } catch (err) {
...
router.post("/", async (req, res) => {
  try {
    const { bookingsDatabase, eventsDatabase, coursesDatabase } = await connectToDatabase();
    const bookingData = req.body;

    // CHANGE: New validation for payment flow
    if (!bookingData.userEmail || !bookingData.productId || !bookingData.productType) {
    ...
    if (existingBooking) {
      return res.status(400).json({ error: "You have already booked this event/course." });
    }
    ...
    if (bookingData.productType === "event") {
      product = await eventsDatabase.findOne({ _id: new ObjectId(bookingData.productId) });
      ...
      product = await coursesDatabase.findOne({ _id: new ObjectId(bookingData.productId) });
